...
var keepOpen=$j('.keepOpen');$j('#mw-panel').addClass('collapsible-nav');$j('#mw-panel > div.portal').each(function(i){var id=$j(this).attr('id');if(keepOpen.length&&keepOpen.attr('id')=='keepOpen-'+id.substring(2)){$j(this).addClass('expanded').find('div.body').show();}else{$j(this).addClass('collapsed');}});function toggle($element){$element.parent().toggleClass('expanded').toggleClass('collapsed').find('div.body').slideToggle('fast');}
var $headings=$j('#mw-panel > div.portal > h5');var tabIndex=mw.usability.getMaxTabIndex()+1;$j('#searchInput').attr('tabindex',tabIndex++);$headings.each(function(){$j(this).attr('tabindex',tabIndex++);});$headings.keydown(function(event){if(event.which==13||event.which==32){toggle($j(this));}}).mousedown(function(){toggle($j(this));$j(this).blur();return false;});});$j(document).ready(function(){if(!wgVectorEnabledModules.collapsibletabs){return true;}
